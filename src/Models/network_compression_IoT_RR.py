...
class Encoder_32_64_iot(Model):
    ...
    def __init__(self, norm_output, **kwargs):
        ...
        self.conv10 = Conv1D(filters=1, kernel_size=7, activation='swish', padding='same',name='AE_encoder_output32CG_compressed')

        self.conv11 = Conv1D(filters=64, kernel_size=7, strides=2, activation='swish', padding='same', name='AE_encoder_output64CG_compressed')
...
def decoder_skip(iot=False):
    ...
    AE_encoder_output_32CG = Conv1D(filters=16, kernel_size=3, activation='swish', padding='same')(AE_encoder_output_32CG)
    AE_encoder_output_32CG_32filters = UpSampling1D(size=2)(AE_encoder_output_32CG)  # [118X16]
    AE_encoder_output_32CG_32filters = Conv1D(filters=16, kernel_size=3, activation='swish', padding='same')(AE_encoder_output_32CG_32filters)  # [63X32]
    AE_encoder_output_32CG_32filters = Conv1D(filters=32, kernel_size=3, activation='swish', padding='valid')(AE_encoder_output_32CG_32filters)  # [124X32]
...
class CompressionAppNetworkSkip(Model):
    ...
    def call(self, inputs, training=False):
        ...
        classification_first = slide_window(self.app_net, out_reconstrcuted_first)
        classification_second = slide_window(self.app_net, out_reconstrcuted_second)
...
def init_compress_app_network(compression_net_builder,compression_weight_net_path,params_config, compile=False, u_net=False):
    ...
    cae = compression_app_network(compression_net, app_model, u_net)
